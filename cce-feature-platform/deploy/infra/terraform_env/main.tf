...
locals {
  name = "cce-feature-platform-${var.environment}"

  common_tags = merge(
    var.tags,
    {
      Application = "cce-feature-platform"
      Environment = var.environment
      ManagedBy   = "terraform"
      SDLCStage   = var.environment
    }
  )
}
...
resource "aws_elasticache_replication_group" "redis" {
  ...
  description                = "Online feature store for CCE feature lookup (${var.environment})"
  engine                     = "redis"
