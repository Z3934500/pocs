...
public class OutboxMetrics {
    ...
    public OutboxMetrics(InventoryOutboxRepository repository, MeterRegistry registry) {
    ...
    public void refresh() {
        List<String> activeStatuses = List.of(InventoryOutboxEvent.PENDING, InventoryOutboxEvent.IN_FLIGHT);
        depth = repository.countByStatusIn(activeStatuses);
        pendingCount = repository.countByStatusIn(List.of(InventoryOutboxEvent.PENDING));
        inFlightCount = repository.countByStatusIn(List.of(InventoryOutboxEvent.IN_FLIGHT));
        deadLetterCount = repository.countByStatusIn(List.of(InventoryOutboxEvent.DEAD_LETTER));
