...
public interface InventoryOutboxRepository extends JpaRepository<InventoryOutboxEvent, UUID> {
    ...
    @Query("select min(event.createdAt) from InventoryOutboxEvent event where event.status in :statuses")
    ...
    @Query("""
        ...
        """)
    List<InventoryOutboxEvent> findClaimable(
            @Param("pending") String pending,
            @Param("inFlight") String inFlight,
    ...
    @Query("""
        update InventoryOutboxEvent event
           set event.status = case when event.attemptCount >= :maxAttempts then :deadLetter else :pending end,
