...
public interface OrderOutboxRepository extends JpaRepository<OrderOutboxEvent, UUID> {
    List<OrderOutboxEvent> findTop100ByStatusOrderByCreatedAtAsc(String status);
    long countByStatusIn(List<String> statuses);

    @Query("select min(event.createdAt) from OrderOutboxEvent event where event.status in :statuses")
    ...
    @Query("""
        ...
        """)
    List<OrderOutboxEvent> findClaimable(@Param("pending") String pending, @Param("inFlight") String inFlight,
                                         @Param("now") LocalDateTime now, Pageable pageable);
    @Query("""
        select count(event) from OrderOutboxEvent event
        where event.eventType in :eventTypes
          and event.partitionKey = :partitionKey
          and event.createdAt < :createdAt
          and event.status <> :published
        """)
    long countEarlierUnpublishedReservationCommands(
    ...
    @Query("""
        update OrderOutboxEvent event set event.status = :inFlight, event.leaseId = :leaseId,
    ...
    @Modifying @Transactional
    @Query("""
        update OrderOutboxEvent event set event.status = :published, event.publishedAt = :publishedAt,
    ...
    @Modifying @Transactional
    @Query("""
        update OrderOutboxEvent event set event.status = case when event.attemptCount >= :maxAttempts
