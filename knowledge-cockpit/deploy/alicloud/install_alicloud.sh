...
if [[ "${EUID}" -ne 0 ]]; then
  echo "Run as root or with sudo: sudo APP_DIR=${APP_DIR} bash knowledge-cockpit/deploy/alicloud/install_alicloud.sh"
  exit 1
fi
...
if [[ ! -f /etc/knowledge-cockpit.env ]]; then
  install -m 600 knowledge-cockpit/deploy/alicloud/knowledge-cockpit.env.example /etc/knowledge-cockpit.env
  echo "Created /etc/knowledge-cockpit.env. Edit OPENAI_API_KEY before using AI KB."
fi

install -m 644 knowledge-cockpit/deploy/alicloud/knowledge-cockpit.service /etc/systemd/system/${SERVICE_NAME}.service
...
Next:
  1. Add Nginx locations from:
     ${APP_DIR}/knowledge-cockpit/deploy/alicloud/nginx-knowledge-cockpit.locations.conf
  2. Confirm Cloudflare Tunnel routes your-own-domain.example to http://localhost:80
